...
/**
 * isInternetConnected() checks for an internet connection via DNS lookup.
 */

BasicManager.prototype.isInternetConnected = function() {
...
/**
 * isConnected() checks the status of the wireless network adapter
 * to determine if it is UP or DOWN
 *
 * @param   interface                   Network interface to check, defaults to wlan0.
 * @returns interfaceConnectionState    The status of the interface (up, down, undefined)
 */

var interfaceConnectionState;

BasicManager.prototype.interfaceConnectionState = function(interface) {
    interface = interface || 'wlan0';
    interfaceConnectionState = '';
    var connectionState = require('child_process').exec;
    var command = connectionState('cat sys/class/net' + interface + '/operstate');

    connectionState.stdout.on('data', function(data) {
        interfaceConnectionState = data;
    });

    return interfaceConnectionState;
};
